...
class User
{
    ...
    // Metoda za updatanje maila in vzdevka
    ...
    //metoda za profile statistiko
    public static function countArticlesByUserId($userId) {
        $db = Db::getInstance();
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM articles WHERE user_id = ?");
    ...
    //metoda za profile statistiko
    public static function countCommentsByUserId($userId) {
        $db = Db::getInstance();
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM comments WHERE user_id = ?");
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $result = $stmt->get_result()->fetch_assoc();
        return $result['count'];
    ...
    //dodatna
    public static function changePasswords($userId, $oldPassword, $newPassword) {
        $db = Db::getInstance();
