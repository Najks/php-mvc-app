...
$controllers = array(
  'pages' => ['error'],
  'users' => ['create', 'store', 'edit', 'update', 'profile', 'changePassword'],
  'auth' => ['login', 'authenticate', 'logout'],
  'articles' => ['index', 'show', 'create', 'store', 'edit', 'update', 'delete', 'list', 'myArticles', 'addComment']
);
// Če je prijavljen, mu dovolimo še urejanje profila, odjavo in objavo novic
if(isset($_SESSION["USER_ID"])){
  $controllers['users'] = array_merge($controllers['users'], ['edit', 'update', 'profile', 'changePassword']);
  $controllers['auth'] = array_merge($controllers['auth'], ['logout']);
  $controllers['articles'] = array_merge($controllers['articles'], ['create'],['addComment']);
}
